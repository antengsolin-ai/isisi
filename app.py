...
async def generate_jwt_token(uid, password):
    """Generate JWT token from ff-jwt-gen-api"""
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url, timeout=24) as response:
                ...
                if response.status != 200:
                    print(f"⚠️ JWT API HTTP {response.status} for UID {uid}: {raw[:200]}")
                    return None

                try:
                    data = json.loads(raw)
                except Exception:
                    print(f"⚠️ JWT API non-JSON response for UID {uid}: {raw[:200]}")
                    return None

                if isinstance(data, dict) and data.get("success") and isinstance(data.get("token"), str):
                    return data["token"]

                print(f"⚠️ JWT API error for UID {uid}: {data}")
                return None
    except Exception as e:
        print(f"❌ JWT exception for UID {uid}: {e}")
        return None
